...
// Simple formatting - strips HTML tags but could be enhanced
function formatMnemonic(text: string): string {
  // Remove common HTML tags from WaniKani mnemonics
  return text
    .replace(/<radical>(.*?)<\/radical>/g, "$1")
    .replace(/<kanji>(.*?)<\/kanji>/g, "$1")
    .replace(/<vocabulary>(.*?)<\/vocabulary>/g, "$1")
    .replace(/<reading>(.*?)<\/reading>/g, "$1")
    .replace(/<ja>(.*?)<\/ja>/g, "$1")
    .replace(/<br\s*\/?>/g, "\n")
    .replace(/<[^>]*>/g, "")
    .trim()
